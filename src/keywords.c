...
char* CurveTypeWords[]   = {w_STORAGE,  w_DIVERSION, w_TIDAL, w_RATING,
                          w_CONTROLS, w_SHAPE,     w_PUMP1, w_PUMP2,
                          w_PUMP3,    w_PUMP4,     NULL};
char* DividerTypeWords[] = {w_CUTOFF, w_TABULAR, w_WEIR, w_OVERFLOW, NULL};
char* EvapTypeWords[]    = {w_CONSTANT, w_MONTHLY,  w_TIMESERIES, w_TEMPERATURE,
                         w_FILE,     w_RECOVERY, w_DRYONLY,    NULL};
...
char* InfilModelWords[]   = {w_HORTON,       w_MOD_HORTON,
                           w_GREEN_AMPT,   w_MOD_GREEN_AMPT,
                           w_CURVE_NUMEBR, NULL};    //(5.1.010)
...
char* NodeTypeWords[]      = {w_JUNCTION, w_OUTFALL, w_STORAGE, w_DIVIDER};
...
char* OutfallTypeWords[] = {w_FREE,  w_NORMAL,     w_FIXED,
                            w_TIDAL, w_TIMESERIES, NULL};
...
char* ReportWords[]       = {w_INPUT,    w_CONTINUITY, w_FLOWSTATS,
                       w_CONTROLS, w_SUBCATCH,   w_NODE,
                       w_LINK,     w_NODESTATS,  NULL};
char* RouteModelWords[] = {w_NONE,     w_STEADY,  w_KINWAVE,
                           w_XKINWAVE, w_DYNWAVE, NULL};
char* RuleKeyWords[] = {w_RULE, w_IF,   w_AND,      w_OR,
                        w_THEN, w_ELSE, w_PRIORITY, NULL};
char* SectWords[] = {
    ws_TITLE,      ws_OPTION,      ws_FILE,        ws_RAINGAGE,
    ws_TEMP,       ws_EVAP,        ws_SUBCATCH,    ws_SUBAREA,
    ws_INFIL,      ws_AQUIFER,     ws_GROUNDWATER, ws_SNOWMELT,
    ws_JUNCTION,   ws_OUTFALL,     ws_STORAGE,     ws_DIVIDER,
    ws_CONDUIT,    ws_PUMP,        ws_ORIFICE,     ws_WEIR,
    ws_OUTLET,     ws_XSECTION,    ws_TRANSECT,    ws_LOSS,
    ws_CONTROL,    ws_POLLUTANT,   ws_LANDUSE,     ws_BUILDUP,
    ws_WASHOFF,    ws_COVERAGE,    ws_INFLOW,      ws_DWF,
    ws_PATTERN,    ws_RDII,        ws_UNITHYD,     ws_LOADING,
    ws_TREATMENT,  ws_CURVE,       ws_TIMESERIES,  ws_REPORT,
    ws_COORDINATE, ws_VERTICES,    ws_POLYGON,     ws_LABEL,
    ws_SYMBOL,     ws_BACKDROP,    ws_TAG,         ws_PROFILE,
    ws_MAP,        ws_LID_CONTROL, ws_LID_USAGE,   ws_GWF,    //(5.1.007)
    ws_ADJUST,     ws_EVENT,                                  //(5.1.011)
    NULL};
char* SnowmeltWords[] = {w_PLOWABLE, w_IMPERV, w_PERV, w_REMOVAL, NULL};
char* TempKeyWords[]  = {w_TIMESERIES, w_FILE, w_WINDSPEED,
                        w_SNOWMELT,   w_ADC,  NULL};
